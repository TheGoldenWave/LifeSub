...
### 4.1 平衡安全与速度
- 每个 Task 最多进行两轮审查：规格复审一次，代码质量/安全复审一次。
- 第二轮之后只处理新发现的 Critical 或 Important；没有新证据不得重复打开已解决的问题。
- Critical：数据损坏、权限绕过、身份/来源伪造、不安全发布或不可恢复安全失败，必须阻塞并修复。
- Important：核心验收失败或重大行为缺陷；若明确属于威胁模型外，记录假设并交由主 Agent 裁定。
- Minor：命名、文件大小、可维护性和非阻断测试增强，记录到 notes，不阻塞当前 Task。
