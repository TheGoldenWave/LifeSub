...
- 归档安装采用 exact whitelist：校验不可变归档 hash 后仍扫描归档全部 entry，对绝对路径、`..`、重复路径、非 UTF-8、symlink、hardlink 和其他特殊 entry fail closed；只把 manifest 固定的 required path 写入 staging。README、LICENSE、导出脚本等文档/脚本，以及 Whisper 可选 INT8 encoder/decoder，只扫描不写入。Qwen3-ASR 0.6B 必须写入完整 tokenizer 三件套 `merges.txt`、`tokenizer_config.json`、`vocab.json`。
- 每个 required path 同时固定精确 bytes 与 SHA-256；安装时逐文件按声明长度限流写入并复验 hash。每个归档还固定 `max_scanned_entries`、`max_written_file_bytes` 与 `max_total_written_bytes`，数值直接来自已验证的不可变归档及 required inventory；任何超限、缺失、重复或漂移均失败，禁止使用通用压缩倍率估算。
- Qwen3-ASR 1.7B 与 Silero VAD 是 direct artifact bundle：分别固定 5 个文件/`4,710,022,180` B 与 1 个文件/`643,854` B 的 exact whitelist、bytes 和 hash，不经过 archive extractor。
- `ArchiveInstallConstraints`、direct bundle 的 max/count summary 和归档 required inventory 是由已经冻结的 artifact bytes/hash 派生的安装安全元数据，不进入 RFC 8785 JCS identity；Qwen3-ASR 1.7B 既有 canonical `ArtifactFile` path/bytes/hash 仍属于 JCS content identity。补齐或校正这些派生约束不得改变当前 manifest versions，也不得改变 Qwen3-ASR 1.7B 的 `8a5c…` identity；只有 content identity 字段发生变化时才发布新 manifest version/identity。validator 必须逐值断言派生约束与冻结表完全一致，防止实现漂移。
- 正式目录不直接写入；所有 bundle 文件分别下载到可恢复 checkpoint，经逐文件校验后在与 final directory 相同 volume 的 staging 组装，写入含 exact whitelist inventory 的安装 marker 并原子 rename。磁盘预检的 additional free requirement 为 `remaining_part_bytes + peak_additional_assembly_bytes + 512 MiB reserve`：已落盘的 verified parts/checkpoints 与旧安装已反映在当前 available space 中，不重复计入；同卷 staging -> final rename 只需要一份 required inventory total，不把 staging/final 计算为两份。使用 checked arithmetic，禁止 `compressed_size * 4` 或其他压缩倍率。
