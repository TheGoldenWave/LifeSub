...
const MIGRATED_BASE_SCHEMA: &str = "
CREATE TABLE sessions (
  id TEXT PRIMARY KEY, title TEXT NOT NULL, state TEXT NOT NULL,
  started_at TEXT NOT NULL, ended_at TEXT
);
...
const FTS_SHADOW_SCHEMA: [(&str, &str); 5] = [
    (
        "segment_search_config",
...
fn classify_locked(connection: &Connection) -> rusqlite::Result<SchemaKind> {
...
fn is_v2(connection: &Connection) -> rusqlite::Result<bool> {
    ...
    if !base_tables_are_exact(connection)? {
        return Ok(false);
    }
...
fn base_tables_are_exact(connection: &Connection) -> rusqlite::Result<bool> {
    for table in ["sessions", "revisions", "segments", "chunks"] {
        let actual = compact_sql(&schema_sql(connection, table)?);
        let fresh = compact_sql(&statement_named(FRESH_BASE_SCHEMA, table)?);
        let migrated = compact_sql(&statement_named(MIGRATED_BASE_SCHEMA, table)?);
        if actual != fresh && actual != migrated {
            return Ok(false);
        }
    }
    Ok(true)
