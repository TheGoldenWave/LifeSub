...
use crate::service::{parse_evidence_uri, EvidenceService, EvidenceTarget};
...
fn markdown_export_contains_revision_and_stable_evidence() {
    ...
    let revision = catalog.append_revision(
        &session.id,
        "demo-local",
        vec![TranscriptSegment::new(1200, 4400, AudioSource::Microphone, "原始音频先持久化")],
    ).unwrap();
    let service = EvidenceService::new(catalog, data_dir.path());
...
fn evidence_uri_parser_keeps_audio_time_ranges() {
    assert_eq!(
        ...
        EvidenceTarget::Audio { id: "chk_123".into(), start_seconds: Some(120), end_seconds: Some(165) }
    );
}
